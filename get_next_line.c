...
/*   Updated: 2023/01/17 13:36:15 by fgalan-n         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0)
		return (NULL);
	static_buffer = ft_file_reader(fd, static_buffer);
	if (!static_buffer)
		return (NULL);
	buffer = ft_new_line(static_buffer);
	static_buffer = ft_upcoming_buffer(static_buffer);
	return (buffer);
}
